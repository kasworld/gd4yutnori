...
func 판위말고르기()->Array[말]:
	for m in 말들:
		if m.판위말인가() and m.지나온눈들[-1].말보기().size() != 0 : # 업힌말 인경우 처리
...
func 판위의말이동하기(이동거리 :int)->이동결과틀:
	...
	if 이동거리 == 0:
		...
		return 결과
	if 이동거리 < 0: # 뒷도개걸 처리
		이동할말들 = 이동할말들[0].지나온눈들[-1].말빼기() # 눈에서 제거한다.
		if 이동할말들[0].지나온눈들.size() <= -이동거리: #판에서 빼서 놓을 말로 돌아간다.
			for i in 이동할말들[0].지나온눈들:
		...
		for i in range(이동거리,0): # 업은말의 첫말의 지나온눈들에서 빼면서 뒤로 이동한다.
			결과.말이동과정눈번호.append(이동할말들[0].지나온눈들.pop_back().번호)
		결과.말이동과정눈번호.append(이동할말들[0].지나온눈들[-1].번호)
		결과.잡힌말들 = 이동할말들[0].지나온눈들[-1].말놓기(이동할말들)
		놓을말로되돌리기(결과.잡힌말들)
		결과.성공 = true
		return 결과

	# 앞으로 가기
	var 기존위치눈번호 = 이동할말들[0].지나온눈들[-1].번호
	결과.말이동과정눈번호 = 길.말이동과정찾기(이동할말들[0].지나온눈들[-1].번호,이동거리)
	if 결과.말이동과정눈번호.size() == 0:
		print("말이동과정찾기 실패 ",이동할말들,결과.말이동과정눈번호)
		return 결과
	이동할말들 = 이동할말들[0].지나온눈들[-1].말빼기() # 눈에서 제거한다.
	for i in 결과.말이동과정눈번호: # 말에 지나가는 눈들 추가
		이동할말들[0].지나온눈들.append(눈들.눈얻기(i))
